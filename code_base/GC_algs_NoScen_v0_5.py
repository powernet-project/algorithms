"""Global Controller Algorithms
	...
	Version Changes:
	- Uses U + R for the bounds
	- Removes Rup Rdown from changing battery SOC (good)
	- Removes Rup Rdown from cost of electricity in scheduler (good)
	- (may have to add penalty for U at time periods with regulation scheduled in ramp preparation step)
...
class Global_Controller(object):
	...
	def StorageFlex_Out(self, network, sScenarios, pDemand, qDemand, prices, boundIndicator, scens, pool, V_weight):
		...
		sols = pool.map(self.StorageFlex_star, itertools.izip(itertools.repeat(network), demandList, itertools.repeat(qDemand), itertools.repeat(prices), itertools.repeat(boundIndicator), itertools.repeat(V_weight))) 
	...
	def flexBounds(self, sScenarios, pForecast, rForecast, pricesCurrent, GCscens, V_weight):
		...
		Ubound_min, rootV2, Wn, Wre, Wie = self.StorageFlex_Out(self.network, sScenarios, pForecast, rForecast, pricesCurrent, boundIndicator, GCscens, self.pool, V_weight)
		boundIndicator = 1 # for maximum bounnd
		Ubound_max, rootV2, Wn, Wre, Wie = self.StorageFlex_Out(self.network, sScenarios, pForecast, rForecast, pricesCurrent, boundIndicator, GCscens, self.pool, V_weight)
	...
	def buildRampRequirements(self, ramp_curr):
		...
		for ramp_key in ramp_curr:
			...
			if np.any(Rtimes >= (GCtime + lookAheadTime)): # if the last ramp extends past the look ahead horizon, ignore it
				print('ramp past horizon')
				continue
			# the optimization checks feasibility so we don't need to do it here
			U_ramp_test, UboundFlag = self.Distribute_Ramp(rampObj.mag, Ubound_max[:,Rtimes], Ubound_min[:,Rtimes], qmax, qmin, umax, umin)
			if np.any(np.isnan(U_ramp_test)):
				...
				print('ramp bound violated so taking note', ramp_key) # ignore ramp when bounds violated
				self.Ubounds_vio.append(ramp_key)
				#self.rampSkips.append(ramp_key)
				#rampUAll.pop(ramp_key)
		...
		if QiList: # if it is not empty
			...
			print('no valid ramps in period')
			self.rampFlag = 0
	...
	def Distribute_Ramp(self, Uramp, Ubound_max, Ubound_min, qmaxo, qmino, umax, umin, q0=np.nan):
		...
		if not np.any(np.isnan(q0)): # if there is a value for q0 use it
			if np.any(np.less(q0, qmino)): # Correct for computational inaccuracies
				...
				q0 += -.00001
			constraints.append( Q[:,0] >= q0 - 1e-6 )
			constraints.append( Q[:,0] <= q0 + 1e-6 )

		
		#constraints.append( sum_entries(U, axis=0) <= Uramp + 0.05*np.absolute(Uramp) )
		#constraints.append( sum_entries(U, axis=0) >= Uramp - 0.05*np.absolute(Uramp) ) # follow within 5% accuracy since equality constraint is unstable
		constraints.append( sum_entries(U, axis=0) <= Uramp + acc )
		constraints.append( sum_entries(U, axis=0) >= Uramp - acc )
		#constraints.append( acc >= 0)
		...
		#obj = Minimize( norm(pos(U - (Ubound_max-tol)) + pos((Ubound_min+tol) - U), 'fro') + (0.001)*norm(U, 1) )
		obj = Minimize( norm(pos(U - (Ubound_max-tol)) + pos((Ubound_min+tol) - U), 'fro') + (0.001)*norm(U, 1) + 100*acc )
		...
		if np.any(np.greater(U.value, Ubound_max)):
			print('U_dist Upper Bound violated')
			boundsFlag = 1
		if np.any(np.less(U.value, Ubound_min)):
		...
		if prob.status != "optimal":
			...
			U_out = U.value
			print('Ramp following error %:', acc.value/np.mean(Uramp))

		return U_out, boundsFlag

	def RampPrep_NLFC_Out(self, network, sScenarios, pDemand, qDemand, q0, prices, sellFactor, scens, pool, V_weight, ramp_weight, RstartList, QiList, RsignList):
		"""
		...
		#pool = Pool()
		print 'Computing in pool'
		sols = pool.map(self.RampPrep_NLFC_star, itertools.izip(itertools.repeat(network), demandList, itertools.repeat(qDemand), itertools.repeat(q0), itertools.repeat(prices), itertools.repeat(sellFactor), itertools.repeat(V_weight), 
			 itertools.repeat(ramp_weight), itertools.repeat(RstartList), itertools.repeat(QiList), itertools.repeat(RsignList))) 
	...
	def RampPrep_NLFC(self, network, pDemand, qDemand, q0, prices, sellFactor, V_weight, ramp_weight, RstartList, QiList, RsignList):
		"""
		...
		if sellFactor == 0:
			...
			obj = Minimize( sum_entries(mul_elemwise(prices, neg(realS[network.battnodes,:]))) # cost min
						 + V_weight*sum_entries(square(pos(Wn - network.V2upBound)) + square(pos(network.V2lowBound - Wn))) # voltage deviations
						 + ramp_weight*norm(max_elemwise(0, mul_elemwise(RsignList,(Q[:,RstartList] - QiList))), 'fro') ) # ramp preparation
		else:
		...
		return realS[network.battnodes,:].value, Wn[network.root,:].value, prob.status, Wn.value, Wre.value, Wie.value, Q.value
	...
	def GC_NLFC_Out(self, network, sScenarios, pDemand, qDemand, q0, prices, sellFactor, scens, pool, V_weight):
		...
		#pool = Pool()
		print 'Computing in pool'
		sols = pool.map(self.GC_NLFC_star, itertools.izip(itertools.repeat(network), demandList, itertools.repeat(qDemand), itertools.repeat(q0), itertools.repeat(prices), itertools.repeat(sellFactor), itertools.repeat(V_weight))) 
	...
	def NSC_Bounds_Out(self, network, pDemand, qDemand, Sload, pool, V_weight):
		...
		#pool = Pool()
		print 'Computing in pool'
		sols = pool.map(self.NSC_Bounds_star, itertools.izip(itertools.repeat(network), itertools.repeat(pDemand), itertools.repeat(qDemand), tNodes, itertools.repeat(Sload), obj_sign, itertools.repeat(V_weight)) ) 
	...
	def NSC_Bounds(self, network, pDemand, qDemand, tNode, Sload, obj_sign, V_weight):
		...
		obj = Minimize( obj_sign*sum_entries(realS[network.battnodes[tNode],:]) + V_weight*sum_entries(square(pos(Wn - network.V2upBound)) + square(pos(network.V2lowBound - Wn))) )
	...
	def RegSchedule(self, network, sScenarios, pDemand, qDemand, prices, ramp_prices_u, ramp_prices_d, reg_prices, V_weight, q0):
		"""
		Determining ramp flexibility controller for cloud coordinator global control
		Inputs: network - Network object
				prices - vector of prices for the times in pDemand
		...
		umin = np.tile(network.umin, (1,T))
		umax = np.tile(network.umax, (1,T))
		qmax = np.tile(network.qmax, (1,T))
		qmin = np.tile(network.qmin, (1,T))
		rYbus = network.realYbus
		iYbus = network.imagYbus
		"""
		if self.sellFactor == 0:
			prices = np.tile(prices, (nS,1))
		"""
		prices = np.tile(prices, (nS,1))
		reg_prices = np.tile(reg_prices, (nS,1))
		ramp_prices_u = np.tile(ramp_prices_u, (nS,1))
		ramp_prices_d = np.tile(ramp_prices_d, (nS,1))
		
		# Make 2 copies of PF constraints for up and down reg signals
		realS = {}
		imagS = {}
		Wre = {}
		Wie = {}
		Wn = {}
		for i in range(GCscens):
			for pfc in range(2):
				realS[2*i+pfc] = Variable(n,T)
				imagS[2*i+pfc] = Variable(n,T)
				Wre[2*i+pfc] = Variable(nE,T)
				Wie[2*i+pfc] = Variable(nE,T)
				Wn[2*i+pfc] = Variable(n,T)
		...
		obj_constraint = Variable(2,2*GCscens)
		...
		obj = [-sum_entries(mul_elemwise(reg_prices, eps)) ]
		obj.append( -sum_entries(mul_elemwise(ramp_prices_u, Rup)) )
		obj.append( -sum_entries(mul_elemwise(ramp_prices_d, Rdown)) )
		
		# 2 PF constraints for up and down ramps
		for i in range(GCscens):
			constraints.append( realS[2*i+0][network.battnodes,:] == -U - sScenarios[i] - Rup - eps )
			constraints.append( realS[2*i+1][network.battnodes,:] == -U - sScenarios[i] + Rdown + eps )
			for pfc in range(2):
				# Demand and battery action constraints
				constraints.append( realS[2*i+pfc][network.nbattInd,:] == -pDemand[network.nbattInd,:] )
				constraints.append( imagS[2*i+pfc][network.nrootInd,:] == -qDemand[network.nrootInd,:] )
				...
				for node in range(n):			
					...
					js = network.nodeNeighbor[node]
					direction = network.nodeDirection[node]
					constraints.append( realS[2*i+pfc][node,:] == rYbus[node,node]*Wn[2*i+pfc][node,:] 
										+ rYbus[node,js]*Wre[2*i+pfc][eidxs,:] + mul_elemwise(direction, iYbus[node,js])*Wie[2*i+pfc][eidxs,:] )
					constraints.append( imagS[2*i+pfc][node,:] == -iYbus[node,node]*Wn[2*i+pfc][node,:] 
									- iYbus[node,js]*Wre[2*i+pfc][eidxs,:] + mul_elemwise(direction, rYbus[node,js])*Wie[2*i+pfc][eidxs,:] )

				# SDP constraint
				for e in range(nE):
					for t in range(T):
						constraints.append( quad_over_lin(Wre[2*i+pfc][e,t], Wn[2*i+pfc][network.nodeElist0[e],t]) 
							+ quad_over_lin(Wie[2*i+pfc][e,t], Wn[2*i+pfc][network.nodeElist0[e],t]) - Wn[2*i+pfc][network.nodeElist1[e],t] <= 0 )

						# annulus constraint
						constraints.append( norm(vstack(Wre[2*i+pfc][e,t], Wie[2*i+pfc][e,t])) <= network.Vmax2 )

				# enforce substation voltage
				constraints.append( Wn[2*i+pfc][network.root,:] == 1.022**2 ) # as per 123 bus case file

				# objective constraints
				obj.append( V_weight*sum_entries(square(pos(Wn[2*i+pfc] - network.V2upBound)) + square(pos(network.V2lowBound - Wn[2*i+pfc])))/2/GCscens )
				if self.sellFactor == 0:
					#obj.append( sum_entries(mul_elemwise(prices, neg(realS[2*i+pfc][network.battnodes,:])))/2/GCscens ) # electricity inclues Rup Rdown
					obj.append( sum_entries(mul_elemwise(prices, neg(-U - sScenarios[i])))/2/GCscens ) # only d+u
				else:
					#obj.append( sum_entries(mul_elemwise(prices, realS[2*i+pfc][network.root,:]))/2/GCscens )
					obj.append( sum_entries(mul_elemwise(prices, U + sScenarios[i]))/2/GCscens ) # only d+u

		obj_full = Minimize( sum(obj) )

		prob = Problem(obj_full, constraints)

		prob.solve(solver = MOSEK)

		return prob.status, U.value, Rup.value, Rdown.value, eps.value
	...
	def runStep(self, q0, t_idx=np.nan):
		...
		netDemand, rDemand, pricesCurrent, reg_prices, ramp_prices_u, ramp_prices_d = self.network.returnGCData(t_idx, GCtime + lookAheadTime)
		...
		print('calculating potential regulation schedule')

		
		start_time = time.time()
		
		prob_status, U, Rup, Rdown, eps = self.RegSchedule(self.network, sScenarios, pForecast, rForecast, pricesCurrent, ramp_prices_u, ramp_prices_d, reg_prices, V_weight, q0)
		if prob_status != "optimal":
			print 'Regulation Scheduling status is: ', prob_status

		print 'regulation schedule comp time', time.time() - start_time
		self.reg_comp = time.time() - start_time
		self.Ubound_min = U - Rdown
		self.Ubound_max = U + Rup
		self.eps = eps
		self.U_sched = U

		
		#self.Ubound_min, self.Ubound_max = self.flexBounds(sScenarios, pForecast, rForecast, pricesCurrent, GCscens, V_weight)
		
		"""
		print self.Ubound_min[0,:]
		print self.Ubound_max[0,:]
		print self.eps[0,:]
		print U[0,:]

		plt.figure()
		#plt.plot(ramp_prices_u.T)
		#plt.plot(pricesCurrent.T)
		plt.plot(np.cumsum(U[0,:]).T)
		plt.figure()
		plt.plot(-Rdown[0,:].T)
		plt.plot(Rup[0,:].T)
		plt.plot(eps[0,:].T)
		...
		#print 'ramp_curr size', ramp_curr.size

		start_time = time.time()

		print('Obtaining net load for ramp preparation')
		if self.rampFlag == 1: # only prepare nodes for ramps if there are ramps in current or look ahead time
			# Prepare storage for ramps
			combDemand = pForecast
			combDemand[self.network.battnodes,:] = sScenarios[0]
			realS, rootV2, prob_status, Wn, Wre, Wie, Q_GC = self.RampPrep_NLFC(self.network, combDemand, rForecast, q0, pricesCurrent, sellFactor, V_weight, ramp_weight, RstartList, QiList, RsignList)
			if prob_status != 'optimal':
				print 'Ramp Prep NLFC status is:', prob_status
			#realS, rootV2, Wn, Wre, Wie, Q_GC = self.RampPrep_NLFC_Out(self.network, sScenarios, pForecast, rForecast, q0, pricesCurrent, sellFactor, GCscens, self.pool, V_weight, ramp_weight, RstartList, QiList, RsignList)
			...
			if np.any(prep_err > .05): # 5% deviation as per arpa-e requirements
				print('prep error is, at time', prep_err, RstartList)
				prob_idx = []
				for err_i in range(prep_err.size): # find problematic ramp
					if prep_err[:,err_i] < .05:
						continue
					else:
			...
			combDemand = pForecast
			combDemand[self.network.battnodes,:] = sScenarios[0]
			realS, rootV2, prob_status, Wn, Wre, Wie = self.GC_NLFC(self.network, combDemand, rForecast, q0, pricesCurrent, sellFactor, V_weight)
			if prob_status != 'optimal':
				print 'Cost Min NLFC status is:', prob_status
			#realS, rootV2, Wn, Wre, Wie = self.GC_NLFC_Out(self.network, sScenarios, pForecast, rForecast, q0, pricesCurrent, sellFactor, GCscens, self.pool, V_weight)

		print 'ramp prep comp time', time.time() - start_time
		self.prep_comp = time.time() - start_time
		...
		return realS, pricesCurrent, LCtime, self.rampFlag, RstartList, QiList, RsignList, ramp_next
	...
	def disaggregateRampSignal(self, t_idx, q0, ramp_next):
		...
		U_ramp_test, UboundFlag = self.Distribute_Ramp(rampObj.mag, self.Ubound_max[:,Rtimes], self.Ubound_min[:,Rtimes], qmax, qmin, umax, umin, q0)
		if UboundFlag == 1:
		...
		if np.any(np.isnan(U_ramp_test)):
			...
			ramp_duration = 0
			U_ramp_test = np.nan
			self.rampSkips.append(ramp_next)
		else:
			self.t_idx = t_idx + ramp_duration

		rampUAll.pop(ramp_next)		

		print 'ramp disaggregate comp time', time.time() - start_time
		self.disag_comp = time.time() - start_time
