...
class Local_Controllers(object):
	...
	def __init__(self, local_data, forecaster, GC_horizon, LCscens, NLweight, sellFactor, ramp_weight, node_idx, q0):

		self.local_data=local_data
		self.forecaster=forecaster
		self.LCscens=LCscens
		self.nodesStorage=node_idx
	...
	def LC_Ramp_No_Bounds(self, realS, NLweight, prices, sellFactor, q0, LCscens, GCtime, pre_pDemands, umaxo, umino, qmaxo, qmino, battnodes, ramp_weight, RstartListo, QiList, RsignList):
		...
		for t in range(GCtime):
			...
			#LCforecast = forecaster then LCforecasts = scenario gen from forecast
			LCforecasts = self.forecaster.scenarioGen(pre_pDemands[:,t:], LCscens, battnodes)
			#print LCforecasts[0].shape
			#print LCforecasts[0]
			if LCforecasts[0].shape[0] > battnodes.size:
				LCforecasts[0]=LCforecasts[0][battnodes,:]
				print 'adjusting LCforecasts'
	...
	def runPeriod(self, t_idx, realS, pricesCurrent, LCtime, rampFlag=0, RstartList=[], QiList=[], RsignList=[]):
		...
		qmax = self.local_data.qmax
		qmin = self.local_data.qmin
		umax = self.local_data.umax
		umin = self.local_data.umin
			
		# Get data from local_data
		# should be historical data when using real forecaster, but this is current data
		preDemand, rDemand, pricesCurrent = self.local_data.returnData(t_idx, GC_horizon)
...
class LocalData(object):
	"""
	object contains all the local data for the LC algorithms
	"""

	def __init__(self, idx, qmax, qmin, umax, umin, netDemandFull, rDemandFull, pricesFull, startIdx=0):
		
		self.startIdx = startIdx
		self.idx = idx # index of this particular local node
		self.qmax = qmax
		self.qmin = qmin
		self.umax = umax
		self.umin = umin
	...
	def returnData(self, t_idx, t_period):
