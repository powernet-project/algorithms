"""Network Data
	...
	Version Changes:
	- Has function that adds random loads to the network
	- Contains regulation signal prices, ramp prices, function to add ramps after initialization
	- Scales ramps so maximum is approximately equal to total storage capacity
"""
...
class Network(object):
	...
	def __init__(self, storagePen, solarPen, nodesPen, pDemandFull, rDemandFull, pricesFull, ramp_prices_uFull, ramp_prices_dFull, reg_pricesFull, root, Ybus, startIdx=0, sNormFull=np.nan, Vmin=0.95, Vmax=1.05, Vtol=0, v_root=1.022, random=True, rampUAll={}):
		...
		self.reg_pricesFull = reg_pricesFull
		self.ramp_prices_uFull = ramp_prices_uFull
		self.ramp_prices_dFull = ramp_prices_dFull
		self.root=root
		self.startIdx=startIdx # starting index of data in pDemandFull and others
		self.rampUAll=rampUAll
		#self.netDemandFull = None

		if random==True:
			print('randomly assigning solar and storage')
	...
	def inputrampUAll(self, rampUAll, ramp_prices_uFull=None, ramp_prices_dFull=None):
		self.rampUAll=rampUAll
		if ramp_prices_uFull is not None:
			self.ramp_prices_uFull=ramp_prices_uFull
		if ramp_prices_dFull is not None:
			self.ramp_prices_dFull=ramp_prices_dFull
	...
	def returnGCData(self, t_idx, t_period, ids='all'):
		if isinstance(ids, str):
			...
			self.reg_pricesFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.ramp_prices_uFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.ramp_prices_dFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)]
		else:
	...
	def returnLCData(self, t_idx, t_period, ids='all'):
		if isinstance(ids, str):
			return self.netDemandFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.rDemandFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.pricesFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
	...
	def returnData(self, t_idx, t_period, ids='all'): # depricated
		if isinstance(ids, str):
			return self.netDemandFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.rDemandFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)], \
			self.pricesFull[:,(t_idx+self.startIdx):(t_idx + t_period + self.startIdx)]
	...
	def addRandomLoad(self, loadPen, load_cap, load_power):

		# randomly pick subset of nondes
		nodesLoad = np.nonzero(self.netDemandFull[:,0])[0]
		if self.netDemandFull[self.root,0] > 0:
			nodesLoad = np.delete(nodesLoad, np.argwhere(nodesLoad==self.root)) # remove substation node
		load_nodes = np.random.choice(nodesLoad, int(np.rint(len(nodesLoad)*loadPen)), replace=False )
		load_nodes = np.sort(load_nodes)

		# find duplicates
		in_first = set(list(self.battnodes))
		in_second = set(list(load_nodes))

		in_second_but_not_in_first = in_second - in_first
		in_both = in_second - in_second_but_not_in_first
		in_both = list(in_both)

		#print self.battnodes

		# To be completed
		"""
		self.qmax[in_both,:] += load_cap
		self.umin[in_both,:] += -load_power
		self.umax[in_both,:] += load_power
		"""

		qmax_n = np.ones((len(in_second_but_not_in_first),1))*load_cap
		qmin_n = np.zeros((len(in_second_but_not_in_first),1))
		umax_n = np.ones((len(in_second_but_not_in_first),1))*load_power
		umin_n = -umax_n

		result = list(self.battnodes) + list(in_second_but_not_in_first)

		self.battnodes = np.array(result)
		self.qmax = np.concatenate((self.qmax,qmax_n))
		self.qmin = np.concatenate((self.qmin,qmin_n))
		self.umax = np.concatenate((self.umax,umax_n))
		self.umin = np.concatenate((self.umin,umin_n))
		#print self.battnodes
		print self.qmax
...
def make_ramp_dict(rampDataAll, windPen, ramp_tolerance, qmax):
	"""
	Make a dictionary of Ramp objects for each ramp
	Inputs: rampDataAll - matrix with ramps X (start/end time start/end power)
		windPen - wind penetration percent used to calculate ramp amount
	...
	if rampDataAll.shape[0] > 1:
		...
		max_idx = np.argmax(rampAmountAll)
		# calculate maximum ramp capacity and scale to qmax
		start, end = rampDataAll[max_idx,[0,1]]
		ramp_cap = rampAmountAll[max_idx]*(np.arange(end-start)+1)/(end-start)
		scale = 0.9*np.sum(qmax)/np.sum(ramp_cap) # largest ramp = 90% of total battery capacity
		...
		for start, end in rampDataAll[:,[0,1]]:
			...
			timesNew = np.arange(start, end)
			# ramp is linearly increasing to maximum value throughout duration
			U = scale*rampAmountAll[idx]*(np.arange(end-start)+1)/(end-start)
		...
		timesNew = np.arange(start, end)
		# ramp is linearly increasing to maximum value throughout duration
		U = np.array(rampAmountAll*(np.arange(end-start)+1)/(end-start))
		U = U.flatten()
		toleranceNew = U*ramp_tolerance
		rampNew = Ramp(timesNew,U,toleranceNew)
		rampUAll[timesNew[0]] = rampNew
